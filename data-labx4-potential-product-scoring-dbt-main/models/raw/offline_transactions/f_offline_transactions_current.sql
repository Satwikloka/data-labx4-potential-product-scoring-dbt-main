...
INNER JOIN {{ source('cds_parquet', 'd_day') }}  dd ON CAST (ftd.the_date_transaction AS DATE) = dd.day_id_day
...
AND ((SELECT MAX(week_id) FROM {{ ref("f_offline_transactions_history") }}) IS NULL OR dd.wee_id_week > (SELECT MAX(week_id) FROM {{ ref("f_offline_transactions_history") }}) )
